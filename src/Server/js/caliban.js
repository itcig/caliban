	...
	window.Caliban = (function () {
			...
			for (i = 0; i < arguments.length; i += 1) {
				...
				if (isString(f)) {
					...
					if (context[f]) {
						...
						var message = 'The method \'' + f + '\' was not found in "_cbn" variable.  Please have a look at the Caliban tracker documentation.';
		...
		function Tracker(trackerUrl, propertyId) {
			...
			function setSessionCookie(sessionReferenceId) {
				setCookie(configSessionIdParam, sessionReferenceId, configSessionTimeout, configCookiePath, configCookieDomain, configCookieIsSecure);
			}
					...
					if (cbn[iterator] && cbn[iterator][0]) {
						...
						if (methodNameToApply === methodName) {
							...
							if (appliedMethods[methodName] > 1) {
								console.warn('The method ' + methodName + ' is registered more than once in "_cbn" variable. Only the last call has an effect. Please have a look at the multiple Caliban trackers documentation: https://developer.caliban.org/guides/tracking-javascript-guide#multiple-caliban-trackers');
							}
		...
		if (typeof define === 'function' && define.amd) {
			define('caliban', [], function () { return Caliban; });
		}
